...
pub use ft_lockup::ft_token_receiver::FtMessage;
...
impl Env {
    ...
    pub fn add_lockup(
        ...
        self.ft_transfer_call(user, amount, &serde_json::to_string(&FtMessage::CreateLockup(lockup.clone())).unwrap())
    }

    pub fn create_initial_lockups(
        &self,
        user: &UserAccount,
        amount: Balance,
        message: &FtMessage,
    ) -> ExecutionResult {
        self.ft_transfer_call(user, amount, &serde_json::to_string(message).unwrap())
    }
